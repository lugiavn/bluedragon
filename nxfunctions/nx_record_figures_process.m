function data = nx_record_figures_process( data )
...
%   Detailed explanation goes here
    ...
    for f=figHandles'
        ...
        try
            ...
            %data.vidObj{f} = VideoWriter([data.path data.name '_' num2str(f) '.avi'], 'Uncompressed AVI');
            data.vidObj{f} = VideoWriter([data.path data.name '_' num2str(f) '.avi']);
            
            %data.vidObj{f}.LosslessCompression = true;
